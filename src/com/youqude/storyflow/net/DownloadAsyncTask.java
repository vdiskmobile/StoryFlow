...
public class DownloadAsyncTask extends AsyncTask<Object[], Object[], Void> {
     ...
     * 必须包含"/"
     */
    private final static String APP_ROOT = "/apps/我的故事/";

    private boolean isShareAction;// false代表分享至新浪微博,true代表保存至百度网盘
    ...
    public DownloadAsyncTask(Context ctx, String access_token, boolean isShareAction, AlbumInfo albumInfo) {
        super();
        this.ctx = ctx;
    ...
    protected void onPostExecute(Void result) {
        ...
        if (mContentLength == downloadFile.length()) {
            ...
            if (tempName.endsWith(Constants.DOWNLOAD_TEMP_FILE_SUFFIX)) {
                ...
                if (isShareAction) {
                    new ShareOperationAsyncTask(ctx, mAlbumInfo).execute(new Object[]{
                          access_token , newFile.getPath()
                    });
                } else {
                    new Uploader(ctx, pcsClient, newFile.getPath(), APP_ROOT, newFile.getName())
                            .execute();
                }
            } else {
